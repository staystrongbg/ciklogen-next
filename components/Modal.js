...
const Modal = ({ src, showmodal }) => {
  const [loc] = useState(useRouter().query.url);
  const [slides] = useState(data.find((u) => u.url === loc)).map(
    (p) => p.photos
  );
  const [error, setError] = useState(false);
  const startIndex = slides.find((slide) => slide.src === src);
  let [index, setIndex] = useState(startIndex.id);

  const handleClick = (dir) => {
    if (dir === 'left') {
      setIndex(index > 0 ? index - 1 : (index = slides.length - 1));
    } else {
      setIndex(index < slides.length - 1 ? index + 1 : (index = 0));
    }
  };
